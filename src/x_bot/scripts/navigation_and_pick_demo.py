...
class NavigationAndPickDemo(Node):
    ...
    def __init__(self):
        super().__init__('navigation_and_pick_demo')
        ...
        self.get_logger().info("=== Navigation and Pick Demo - Real-time YOLOE Edition ===")
    ...
    def navigate_to(self, x, y, yaw_deg, timeout=120.0):
        self.get_logger().info(f">>> Navigating to X={x:.2f}, Y={y:.2f}, Yaw={yaw_deg:.1f}°")
        import math
    ...
    def run_logic_loop(self):
        ...
        self.get_logger().info("\n" + "#"*60)
        self.get_logger().info("### Navigation & Pick Demo ###")
        self.get_logger().info("#"*60)
        
        # 1. 导航到厨房固定点 (X=3.76, Y=2.22, Yaw=6.0°)
        self.get_logger().info("\n>>> STEP 1: Navigate to Kitchen")
        success = self.navigate_to(3.76, 2.22, 6.0)
        if not success:
            self.get_logger().error("Failed to reach kitchen. Aborting.")
            return
        ...
        # 3. 机械臂夹取可乐
        self.get_logger().info("\n>>> STEP 3: Pick Coke")
        picked_count = self.process_targets([9], "Kitchen Table")
        
        if picked_count == 0:
            self.get_logger().warn("Could not pick Coke. Returning anyway.")
        ...
        success = self.navigate_to(1.44, 3.32, 66.9)
        if success:
            self.get_logger().info("\n" + "#"*60)
            self.get_logger().info("### Task Successfully Completed! ###")
            self.get_logger().info("#"*60)
        else:
            self.get_logger().error("Failed to return to base.")
